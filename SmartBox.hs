...
createVBoxVM sb@SmartBox{..} = do
  status (DTL.pack $ "Creating a new SmartOS VBox in " ++ show sbVmDirPath)
    $ vbManage_ CreateVM [ "--name", ident sbVm
  ...
  status (DTL.pack $ "Creating 40GB disk " ++ show sbVmDiskPath)
    $ vbManage_ CreateHD [ "--filename", toTextIgnore sbVmDiskPath
                         , "--size", "40960" ]
  status (DTL.pack $ "Attaching " ++ show sbVmDiskPath)
    $ v StorageAttach [ "--device", "0"
                      , "--medium", toTextIgnore sbVmDiskPath
                      , "--port", "0"
...
updateVBoxVM SmartBox{..} = do
  status (DTL.pack $ "Attaching " ++ show sbIsoPath)
    $ v StorageAttach [ "--device", "0"
